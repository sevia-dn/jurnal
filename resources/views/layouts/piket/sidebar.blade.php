...
<aside class="hidden md:flex flex-col w-64 bg-[#0D6B5A] h-screen fixed left-0 top-0 font-sans border-r border-[#17826E] z-40">
  ...
  <nav class="flex-1 px-4 overflow-y-auto space-y-1.5">
    ...
    <!-- Logbook Mengajar Guru -->
    <a href="{{ route('guru.logbook.create') }}" 
       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors !no-underline {{ request()->routeIs('guru.logbook*') ? 'bg-[#1BA886] !text-white' : '!text-[#8EBEB2] hover:bg-[#1BA886]/10 hover:!text-white' }}">
      <i class="bi bi-journal-check text-lg"></i>
      <span>Logbook Guru Saya</span>
    </a>
