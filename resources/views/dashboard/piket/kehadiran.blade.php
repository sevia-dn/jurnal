        ...
        <div class="mx-auto max-w-7xl">
            <header class="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="flex flex-col gap-3 sm:flex-row">
                    <label class="relative"><span class="sr-only">Pilih tanggal</span><i class="bi bi-calendar3 pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-emerald-700" aria-hidden="true"></i><input id="attendance-date" type="date" value="{{ $tanggal }}" class="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm font-semibold text-slate-700 shadow-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"></label>
                    <button id="export-data" type="button" class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-800 focus:outline-none focus:ring-4 focus:ring-emerald-200 cursor-pointer"><i class="bi bi-download" aria-hidden="true"></i> Export Data CSV</button>
                </div>
            </header>

            <!-- Summary Cards Kehadiran Guru -->
            <section class="mb-7 grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Guru</p>
                    <p class="mt-2 text-2xl font-extrabold text-slate-900">{{ $totalGuru }}</p>
                    <p class="mt-2 text-xs font-semibold text-slate-500">Guru pengajar</p>
                </div>
                <div class="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 shadow-sm">
                    ...
                    <p class="mt-2 text-2xl font-extrabold text-emerald-800">{{ $totalHadir }}</p>
                    <p class="mt-2 text-xs font-semibold text-emerald-600">{{ $totalGuru > 0 ? round(($totalHadir / $totalGuru) * 100) : 0 }}% kehadiran</p>
                </div>
                <div class="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 shadow-sm">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-amber-700">Izin / Sakit</p>
                    <p class="mt-2 text-2xl font-extrabold text-amber-800">{{ $totalIzin + $totalSakit }}</p>
                    <p class="mt-2 text-xs font-semibold text-amber-600">{{ $totalIzin }} Izin, {{ $totalSakit }} Sakit</p>
                </div>
                <div class="rounded-2xl border border-rose-200 bg-rose-50/60 p-4 shadow-sm">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-rose-700">Belum Hadir</p>
                    <p class="mt-2 text-2xl font-extrabold text-rose-800">{{ $totalBelumHadir }}</p>
                    <p class="mt-2 text-xs font-semibold text-rose-600">Belum lapor presensi</p>
                </div>
            </section>
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="attendance-table-title">
                <div class="flex items-center justify-between border-b border-slate-100 px-5 py-5 sm:px-6"><div><h2 id="attendance-table-title" class="font-bold text-slate-800">Daftar Kehadiran</h2><p id="attendance-summary" class="mt-1 text-sm text-slate-500" aria-live="polite"></p></div><span class="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 border border-emerald-200">{{ \Carbon\Carbon::parse($tanggal)->translatedFormat('l, d F Y') }}</span></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const badge = (status) => ({
        ...
        Sakit: 'bg-amber-50 text-amber-700 border border-amber-200',
        Alfa: 'bg-rose-50 text-rose-700 border border-rose-200',
        'Belum Hadir': 'bg-slate-100 text-slate-600 border border-slate-200'
    ...
    function render() {
        ...
        table.innerHTML = items.map((teacher, index) => {
            let actionBtn = `<button type="button" data-detail-user-id="${teacher.user_id}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:bg-slate-100">Detail</button>`;

            if (teacher.status === 'Hadir') {
                if (teacher.verified) {
                    actionBtn = `<span class="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 mr-2"><i class="bi bi-check-all text-sm"></i> Terverifikasi</span>` + actionBtn;
                } else if (teacher.id) {
                    actionBtn = `
                        <form method="POST" action="/dashboard/piket/kehadiran/${teacher.id}/verifikasi" class="inline m-0 p-0">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <button type="submit" class="rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 text-xs font-bold shadow-sm transition mr-1.5 cursor-pointer"><i class="bi bi-check-circle"></i> Verifikasi</button>
                        </form>
                    ` + actionBtn;
                }
            }

            return `<tr class="${index % 2 ? 'bg-slate-50/60' : 'bg-white'} transition hover:bg-emerald-50/50">
                <td class="px-6 py-4 font-bold text-slate-800">${teacher.name}</td>
                <td class="px-6 py-4 font-mono text-xs text-slate-500">${teacher.nip}</td>
                <td class="px-6 py-4 font-semibold text-slate-600">${teacher.checkIn}</td>
                <td class="px-6 py-4"><span class="rounded-full px-3 py-1 text-xs font-bold ${badge(teacher.status)}">${teacher.status}</span></td>
    ...
    table.addEventListener('click', (event) => {
        const button = event.target.closest('[data-detail-user-id]'); if (!button) return;
        const teacher = teachers.find((item) => item.user_id === Number(button.dataset.detailUserId)); if (!teacher) return;
        activeId = teacher.user_id; trigger = button;
