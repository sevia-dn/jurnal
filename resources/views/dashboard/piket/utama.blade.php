...
@section('content')
    <div id="piket-dashboard" class="min-h-full bg-slate-50 p-5 pb-24 font-sans sm:p-8 lg:p-10">
        <div class="mx-auto max-w-7xl">
            ...
            <!-- Tim Piket & Waka Backup Banner -->
            <div class="mb-7 rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-start gap-3.5">
                        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-lg text-emerald-700 shadow-xs">
                            <i class="bi bi-shield-check text-xl"></i>
                        </span>
                        <div>
                            <div class="flex items-center gap-2">
                                <h2 class="text-base font-extrabold text-slate-800">Tim Piket Hari Ini ({{ $namaHari ?? 'Hari Ini' }})</h2>
                                <span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-extrabold text-emerald-800 border border-emerald-300">Siap Bertugas</span>
                            </div>
                            <p class="mt-0.5 text-xs text-slate-500">Petugas piket utama dan dukungan cadangan (*backup*) untuk memvalidasi perizinan & dispensasi.</p>
                        </div>
                    </div>

                    <div class="flex flex-wrap items-center gap-2.5 text-xs">
                        <div class="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2">
                            <span class="font-semibold text-slate-400">Guru Piket:</span>
                            <span class="font-bold text-slate-800">{{ optional(optional($piketGuru)->user)->name ?? 'Belum Ditentukan' }}</span>
                        </div>
                        <div class="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-2">
                            <span class="font-semibold text-emerald-700">Waka Piket:</span>
                            <span class="font-bold text-emerald-900">{{ optional(optional($piketWaka)->user)->name ?? 'Belum Ditentukan' }}</span>
                        </div>
                        <div class="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2" title="Jika waka utama berhalangan, waka lain otomatis dapat membackup validasi/dispensasi">
                            <i class="bi bi-people-fill text-amber-600"></i>
                            <span class="font-semibold text-amber-800">Waka Backup:</span>
