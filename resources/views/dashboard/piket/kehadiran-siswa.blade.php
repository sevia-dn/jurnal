    ...
    <div class="mx-auto max-w-7xl">
        ...
        <section class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="relative flex-1 max-w-md">
                ...
                <input type="text" id="search-input" placeholder="Cari nama atau NISN siswa..." autocomplete="off" class="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-sm font-medium text-slate-800 shadow-sm placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-100">
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const studentsData = [
        { nisn: '0101081001', name: 'Aditya Pratama', gender: 'L', class: 'XI RPL 1', status: 'Hadir', note: '-' },
        { nisn: '0101081002', name: 'Alya Nuraini', gender: 'P', class: 'XI RPL 1', status: 'Hadir', note: '-' },
        { nisn: '0101081003', name: 'Bagus Setyo', gender: 'L', class: 'XI RPL 1', status: 'Sakit', note: 'Surat dokter terlampir' },
        { nisn: '0101081004', name: 'Citra Dewi', gender: 'P', class: 'XI RPL 1', status: 'Hadir', note: '-' },
        { nisn: '0101081005', name: 'Dian Ananda', gender: 'P', class: 'XI RPL 1', status: 'Izin', note: 'Acara keluarga' },
        { nisn: '0101081006', name: 'Eko Prasetyo', gender: 'L', class: 'XI RPL 1', status: 'Alfa', note: 'Tanpa konfirmasi' },
        { nisn: '0101082001', name: 'Fahri Ramadhan', gender: 'L', class: 'XI RPL 2', status: 'Hadir', note: '-' },
        { nisn: '0101082002', name: 'Gita Gutawa', gender: 'P', class: 'XI RPL 2', status: 'Sakit', note: 'Demam tinggi' },
    ];
    ...
    function renderTable() {
        ...
        const filtered = studentsData.filter(s => {
            ...
            const matchSearch = s.name.toLowerCase().includes(searchQuery) || s.nisn.includes(searchQuery);
        ...
        tableBody.innerHTML = filtered.map(s => `
            <tr class="transition hover:bg-slate-50">
                <td class="whitespace-nowrap px-6 py-4 font-mono text-xs text-slate-500">${s.nisn}</td>
                ...
                <td class="px-6 py-4 text-center">
                    <button type="button" onclick="openEditModal('${s.nisn}')" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-200">
                        Ubah Status
                    </button>
                </td>
            </tr>
    ...
    window.openEditModal = function(nisn) {
        selectedStudent = studentsData.find(s => s.nisn === nisn);
        if (!selectedStudent) return;

        document.getElementById('modal-student-name').textContent = selectedStudent.name;
        document.getElementById('modal-student-nis').textContent = `NISN: ${selectedStudent.nisn} • Kelas: ${selectedStudent.class}`;
