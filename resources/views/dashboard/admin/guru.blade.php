...
=======
        <form method="GET" action="{{ route('admin.guru') }}">
            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari NIP, nama, atau mapel..."
...
=======
        ...
        <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-6 mb-8">
            ...
            <form method="POST" action="{{ route('admin.guru.store') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                @csrf
                <div class="flex flex-col">
                    <label class="text-sm font-medium text-gray-700 mb-1">NIP</label>
                    <input type="text" name="nip" placeholder="198005122005011002"
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" name="username" placeholder="budiwaluyo" required
                        class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-400">
                </div>
...
=======
                        ...
                        <td class="p-4 text-center space-x-2">
                            <a href="{{ route('admin.guru.edit', $user->id) }}" class="text-gray-400 hover:text-amber-600 transition inline-block" title="Edit">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                </svg>
                            </a>
                            <form action="{{ route('admin.guru.destroy', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin mau hapus data guru ini?');">
                                @csrf
                                @method('DELETE')
