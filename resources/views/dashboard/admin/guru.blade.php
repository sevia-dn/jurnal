...
<div class="p-6 sm:p-10 font-sans">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-8">
        ...
        <form method="POST" action="{{ route('admin.guru.store') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex flex-col">
                <label class="text-sm font-medium text-gray-700 mb-1">Username <span class="text-red-500">*</span></label>
                <input type="text" name="username" required placeholder="username_guru"
                       class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-400">
            </div>

            <div class="flex flex-col">
                <label class="text-sm font-medium text-gray-700 mb-1">Password <span class="text-red-500">*</span></label>
            ...
            <div class="flex flex-col">
                ...
                <input type="text" name="nama" required placeholder="Budi Waluyo, S.Pd"
                       class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-400">
            </div>
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                @forelse($users as $user)
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">{{ $user->mapel->nama_mapel ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->no_hp ?? '-' }}</td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.guru.edit', $user->id) }}" class="text-gray-400 hover:text-amber-600 transition" title="Edit">
                                    <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                    </svg>
                                </a>
                                <button type="button" onclick="confirmDelete('{{ route('admin.guru.destroy', $user->id) }}')" class="text-gray-400 hover:text-red-600 transition" title="Hapus">
                                    <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="modalHapusGuru" class="fixed inset-0 z-50 hidden bg-gray-900/60 backdrop-blur-sm overflow-y-auto h-full w-full items-center justify-center transition-opacity">
    <div class="bg-white rounded-xl shadow-xl border border-gray-100 p-8 max-w-md w-full mx-4 text-center relative">
        ...
        <p class="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin menghapus data guru ini? Tindakan ini tidak dapat dibatalkan.</p>

        <form id="formHapusGuru" method="POST" class="text-left space-y-4">
            @csrf
            @method('DELETE')
...
<script>
    function confirmDelete(url) {
        document.getElementById('formHapusGuru').action = url;
        openModal('modalHapusGuru');
    }
