...
<div x-data="userManagement()" class="p-6 sm:p-10 font-sans">
    ...
    <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex flex-col gap-3 md:flex-row">
            ...
            <div class="relative flex-1">
                ...
                <input x-model="search" type="search" autocomplete="off" placeholder="Cari nama, username, NIP, kelas, mapel, atau no HP..." class="w-full rounded-lg border border-gray-300 py-2.5 pl-10 pr-4 text-sm outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20">
            </div>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <tr>
                        ...
                        <th class="px-5 py-4">
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800" title="Diurutkan berdasarkan abjad A-Z">
                    ...
                    <template x-for="(user, index) in filteredUsers" :key="user.id">
                        <tr class="transition-colors hover:bg-slate-50/70">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <template x-if="user.nama_mapel && user.raw_role === 'guru'">
                            ...
                            <td class="px-5 py-4 font-mono text-gray-600">
                                <div class="font-semibold text-gray-800" x-text="user.username"></div>
                                <template x-if="user.nip">
                                    <div class="text-[11px] text-gray-400 font-sans">NIP: <span x-text="user.nip"></span></div>
                                </template>
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="user.roleClass" x-text="user.role"></span>
                            </td>
                            <td class="px-5 py-4 text-gray-600" x-text="user.no_hp || user.phone || '-'"></td>
                            ...
                            <td class="px-5 py-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <!-- Tombol Edit -->
                                    <button type="button" @click="openEdit(user)" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 transition hover:bg-[#1BA886]/10 hover:text-[#0f766e] cursor-pointer" title="Edit Akun">
                                        <i class="bi bi-pencil text-sm"></i>
                                    </button>

                                    <!-- Tombol Hapus (Dilindungi untuk Admin Utama) -->
                                    <template x-if="user.id !== 1 && user.username !== 'admin1' && user.username !== 'admin'">
                                        <button type="button" @click="openDelete(user)" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition hover:bg-red-50 hover:text-red-600 cursor-pointer" title="Hapus Akun">
                                            <i class="bi bi-trash3 text-sm"></i>
                                        </button>
                                    </template>
                                    <template x-if="user.id === 1 || user.username === 'admin1' || user.username === 'admin'">
                                        <span class="inline-flex h-8 w-8 items-center justify-center text-gray-300" title="Akun Administrator Utama (Terkunci)">
        ...
        <div class="bg-gray-50 p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-gray-500">
            <div>
                ...
                (<span class="text-emerald-700 font-semibold" x-text="users.length + ' Akun Aktif'"></span>)
            </div>
            <div class="text-gray-400 flex items-center gap-3">
                <span class="flex items-center gap-1.5"><span class="inline-block w-2 h-2 rounded-full bg-purple-500"></span> Admin</span>
                <span class="flex items-center gap-1.5"><span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span> Guru Pengajar</span>
                <span class="flex items-center gap-1.5"><span class="inline-block w-2 h-2 rounded-full bg-sky-500"></span> Sekretaris Kelas</span>
            </div>
        </div>
    </div>
    ...
    <div x-show="addModalOpen" x-cloak x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-xs" @keydown.escape.window="closeAdd()" @click.self="closeAdd()">
        <div x-show="addModalOpen" x-transition class="my-8 w-full max-w-xl rounded-2xl border border-gray-100 bg-white shadow-2xl">
            ...
            <form action="{{ route('admin.manajemen-user.store') }}" method="POST" autocomplete="off" class="p-6 text-xs sm:text-sm">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div :class="(addRole !== 'sekretaris' && addRole !== 'guru') ? 'col-span-1' : ''">
                        ...
                        <input name="username" x-model="addUsername" type="text" required autocomplete="off" placeholder="Contoh: admin2, sekre_xrpl1" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input name="name" x-model="addNama" type="text" required autocomplete="off" placeholder="Masukkan nama lengkap pengguna..." class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-4">
                    <div>
                        ...
                        <input name="no_hp" type="tel" value="{{ old('no_hp') }}" autocomplete="off" placeholder="Contoh: 081234567890" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                    <div>
                        <label class="mb-1.5 block font-semibold text-gray-700">NIP (Jika Guru / ASN)</label>
                        <input name="nip" type="text" value="{{ old('nip') }}" autocomplete="off" placeholder="Nomor NIP..." class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input name="password" type="password" required autocomplete="new-password" placeholder="Minimal 6 karakter..." class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                </div>
    ...
    <div x-show="editModalOpen" x-cloak x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-xs" @keydown.escape.window="closeEdit()" @click.self="closeEdit()">
        <div x-show="editModalOpen" x-transition class="my-8 w-full max-w-xl rounded-2xl border border-gray-100 bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Perbarui data login, role, nomor kontak, atau ganti password.</p>
                    </div>
                </div>
            ...
            <form :action="'{{ url('dashboard/admin/manajemen-user') }}/' + selectedUser.id" method="POST" autocomplete="off" class="p-6 text-xs sm:text-sm">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block font-semibold text-gray-700">Role Sistem <span class="text-red-500">*</span></label>
                        <select name="role" x-model="selectedUser.raw_role" required class="w-full rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 cursor-pointer text-sm">
                            <option value="admin">Admin</option>
                            <option value="guru">Guru Pengajar</option>
                            <option value="pengurus_kelas">Sekretaris Kelas</option>
                        </select>
                    </div>

                    <!-- Mapel (jika role Guru Pengajar) -->
                    <div x-show="selectedUser.raw_role === 'guru'">
                    ...
                    <!-- Username Login -->
                    <div :class="selectedUser.raw_role !== 'guru' ? 'col-span-1' : ''">
                        <label class="mb-1.5 block font-semibold text-gray-700">Username Login <span class="text-red-500">*</span></label>
                        <input name="username" x-model="selectedUser.username" type="text" required autocomplete="off" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input name="name" x-model="selectedUser.name" type="text" required autocomplete="off" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-4">
                    <div>
                        <label class="mb-1.5 block font-semibold text-gray-700">Nomor Handphone</label>
                        <input name="no_hp" x-model="selectedUser.no_hp" type="tel" autocomplete="off" placeholder="Contoh: 081234567890" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                    <div>
                        <label class="mb-1.5 block font-semibold text-gray-700">NIP (Jika Guru / ASN)</label>
                        <input name="nip" x-model="selectedUser.nip" type="text" autocomplete="off" placeholder="Nomor NIP..." class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input name="password" x-model="newPassword" type="password" autocomplete="new-password" placeholder="Kosongkan jika tidak ingin mengubah password" class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 outline-none transition focus:border-[#1BA886] focus:ring-2 focus:ring-[#1BA886]/20 text-sm">
                </div>
    ...
    <div x-show="deleteModalOpen" x-cloak x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs" @keydown.escape.window="closeDelete()" @click.self="closeDelete()">
        <div x-show="deleteModalOpen" x-transition class="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
            <div class="mb-4 flex items-center gap-3 border-b border-slate-100 pb-3">
                ...
                <div>
                    <h2 class="text-base font-bold text-gray-900">Hapus Akun Pengguna</h2>
                    <p class="text-xs text-gray-500">Akun akan dihapus secara permanen dari sistem.</p>
                </div>
            </div>

            <div class="mb-5 p-4 bg-red-50/70 border border-red-200 rounded-xl text-xs text-red-900">
                <span class="block text-gray-500 font-medium">Target Akun yang Dihapus:</span>
                <span class="font-bold text-sm block mt-1 text-gray-900" x-text="selectedUser.name"></span>
                <span class="text-red-700 block mt-0.5" x-text="selectedUser.role + ' • ' + (selectedUser.username || selectedUser.identifier)"></span>
            </div>

            <p class="text-xs text-gray-500 mb-5 leading-relaxed">
                Apakah Anda yakin ingin menghapus akun ini? Akun yang sudah dihapus tidak dapat dipulihkan kembali.
            </p>

            <form :action="'{{ url('dashboard/admin/manajemen-user') }}/' + selectedUser.id" method="POST">
                @csrf
                @method('DELETE')

                <div class="flex justify-end gap-2.5 pt-2 border-t border-slate-100 text-xs">
                    ...
                    <button type="submit" class="rounded-xl bg-red-600 px-5 py-2 font-bold text-white shadow-xs hover:bg-red-700 transition cursor-pointer flex items-center gap-1.5">
...
<script>
    function userManagement() {
        return {
            search: '',
            roleFilter: '',
            addModalOpen: {{ (request('tambah') || (isset($errors) && $errors->any())) ? 'true' : 'false' }},
            editModalOpen: false,
            ...
            addNama: '',
            addUsername: '',
            selectedUser: { id: null, name: '', role: '', identifier: '', phone: '', no_hp: '', nip: '', id_kelas: '', mapel_id: '', username: '', raw_role: '' },
            ...
            openEdit(user) {
                this.selectedUser = {
                    ...user,
                    no_hp: user.no_hp || (user.phone !== '-' ? user.phone : '') || '',
                    nip: user.nip || ''
                };
                this.newPassword = '';
